  ...
  const sendEmail = async (e) => {
    ...
    if (window.Email) {
      window.Email.send({
        ...config,
        To: data.to,
        From: "shubhampeddarpeth@gmail.com",
  ...
  const onFileChange = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        const recipients = contents.split("\n").map((line) => line.trim());
        setData({ ...data, to: recipients.join(", ") });
      };
    ...
    <Dialog open={open} PaperProps={{ sx: dialogStyle }}>
      ...
      <RecipientWrapper>
        <InputBase
          ...
          value={data.to}
        />
        <InputBase type="file" name="file" onChange={onFileChange} />
        <InputBase
          placeholder="Subject"
          name="subject"
